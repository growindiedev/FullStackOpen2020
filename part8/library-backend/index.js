...
const resolvers = {
  Query: {
    ...
    allBooks: () => books,
    allAuthors: () => {
      const booksPerAuthor = authors.map(
        author => books.filter(book => book.author === author.name)
      )
      return booksPerAuthor.map( item => ({
        bookCount: item.length,
        name: item[0].author,
        born: authors.find(author => author.name === item[0].author).born
      }))
    },
  ...
  Mutation: {
    ...
    editAuthor: (root, args) => {
        ...
        // authors.splice(authors.indexOf(author), 1, {...author, born: args.setBornTo})
